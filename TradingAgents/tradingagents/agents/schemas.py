...
class PortfolioDecision(BaseModel):
    ...
    execution_type: str = Field(
        default="limit",
        description=(
            "How to enter the position at the NY open: 'market' (buy/sell "
            "immediately at the open, no guarantee of entry price — use when "
            "the setup is time-sensitive or gaps are likely) or 'limit' "
            "(rest a limit order at the entry price — use when price is "
            "expected to return to the level). 'none' if no entry."
        ),
    )
