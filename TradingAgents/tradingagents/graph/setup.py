...
class GraphSetup:
    ...
    def __init__(
        ...
        ``role_llms`` (spec 2026-08-24 §4/§6): role -> LLM client map for
        per-node model routing. When provided it takes precedence over the
        quick/deep slots; individual roles missing from the map fall back to
        quick (analysts/debate) or deep (manager/PM) as appropriate.
        """
    ...
    def _role_llm(self, role: str, fallback: Any) -> Any:
        """Resolve the client for a role, falling back when unconfigured."""
    ...
    def setup_graph(
        ...
        analyst_factories = {
            "market": lambda: create_market_analyst(
                self._role_llm("market", self.quick_thinking_llm)),
            "social": lambda: create_sentiment_analyst(
                self._role_llm("sentiment", self.quick_thinking_llm)),
            "news": lambda: create_news_analyst(
                self._role_llm("news", self.quick_thinking_llm)),
            "fundamentals": lambda: create_fundamentals_analyst(
                self._role_llm("fundamentals", self.quick_thinking_llm)),
        }
        ...
        trader_node = create_trader(
            self._role_llm("trader", self.quick_thinking_llm))

        # Create risk analysis nodes
        aggressive_analyst = create_aggressive_debator(
            self._role_llm("aggressive", self.quick_thinking_llm))
        neutral_analyst = create_neutral_debator(
            self._role_llm("neutral", self.quick_thinking_llm))
        conservative_analyst = create_conservative_debator(
            self._role_llm("conservative", self.quick_thinking_llm))
